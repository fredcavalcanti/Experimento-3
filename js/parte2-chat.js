...
function Parte2Chat(experimentoChave , idplayer) {
...
Parte2Chat.prototype.getID = function() {
  firebase.database().ref('/experiment/'+this.experimentoChave+'/participant/' + this.idplayer).once('value').then(function(snapshot) {
    ...
    Parte2Chat.btnProximo.setAttribute('href','parte2-decisao.html');
    Parte2Chat.iAmHere();
    verifica();
  });
};
...
Parte2Chat.prototype.iAmHere = function() {
  var tableCtrlChat = firebase.database().ref('/experiment/'+Parte2Chat.experimentoChave+'/ctrlChat2/' + Parte2Chat.sala.value +'/'+ Parte2Chat.idplayer);
  ...
  firebase.database().ref('/experiment/'+Parte2Chat.experimentoChave+'/ctrlChat2/' + Parte2Chat.sala.value).on('value',function(snapshot) {
 ...
 var numerogrupo = firebase.database().ref('/experiment/'+Parte2Chat.experimentoChave+'/participant/' + Parte2Chat.idplayer+ '/group/' + 'number').once('value').then(function(snapshot) {
       var group_number = snapshot.val();

       var adc = firebase.database().ref('/experiment/'+ Parte2Chat.experimentoChave + '/contadorchat2/' + group_number).once('value').then(function(snapshot) {
       var adc_number = snapshot.val();
       var maisum = adc_number + 1 ;

       var acrescenta = firebase.database().ref('/experiment/'+ Parte2Chat.experimentoChave + '/contadorchat2/' + group_number);
        acrescenta.set(maisum);

      var ref = firebase.database().ref('/experiment/'+ Parte2Chat.experimentoChave + '/contadorchat2/' + group_number);
    ...
    if(valorbolado>=3) {
      firebase.database().ref('/experiment/'+Parte2Chat.experimentoChave+'/ctrlChat2/' + Parte2Chat.sala.value).off();
      verifica();
...
firebase.auth().onAuthStateChanged(firebaseUser => {
    if(firebaseUser){
              ...
              window.Parte2Chat = new Parte2Chat(experimentoChave , idplayer);
    }
    else
    {
...
function verifica(){
  ...
  firebase.database().ref('/experiment/'+Parte2Chat.experimentoChave + '/participant/' + Parte2Chat.idplayer + '/group/').once('value').then(function(snapshot) {
    
    var meugrupo = snapshot.val().number;


var idpartida = firebase.database() .ref('/experiment/'+Parte2Chat.experimentoChave + '/ctrlChat2/' + meugrupo +'/');
...
firebase.database().ref('/experiment/'+Parte2Chat.experimentoChave + '/ctrlChat2/' + meugrupo + '/' + Parte2Chat.idplayer).once('value').then(function(snapshot) {
