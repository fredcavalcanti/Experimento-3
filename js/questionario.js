...
function Questionario(experimentoChave , idplayer) {
...
// ERROR 001
  firebase.database().ref('/experiment/'+this.experimentoChave+'/participant/').startAt(Questionario.idplayer).once('value', function(snapshot) {
    
    var participants = snapshot.val();
    console.log('Q: ' + participants);
    var qtd=1;
    var numeroGrupo=1;
    
    for(var p in participants) {
      console.log("P: "+p);
      if(p == Questionario.idplayer) {
    ...
    var table = firebase.database().ref('/experiment/'+Questionario.experimentoChave+'/participant/'+Questionario.idplayer+'/group');
    ...
    tableGrupo.push(Questionario.idplayer).then(function(snapshot) {
...
Questionario.prototype.send = function() {
  ...
  if(qtdError==0) {
    this.table = firebase.database().ref('/experiment/'+Questionario.experimentoChave+'/participant/'+Questionario.idplayer+'/quiz/');
    ...
    this.table.set(answer).then(function(snapshot) {
      ...
      var qual_grupo = firebase.database().ref("/experiment/" + Questionario.experimentoChave+'/participant/' + Questionario.idplayer + '/group/tipo').once('value').then(function(snapshot) {
       var qualgrupo = snapshot.val();;
  
   if(qualgrupo == "PC"){
    window.location.href="parte1-instrucoes-pc.html";
   }
   else{
    window.location.href="parte1-instrucoes-npc.html";
   }
});
 ...
 firebase.auth().onAuthStateChanged(firebaseUser => {
    if(firebaseUser){
              ...
              window.Questionario = new Questionario(experimentoChave , idplayer);
    }
    else
    {
...
function des()
{
  ...
  var qual_curso = firebase.database().ref("/experiment/" + Questionario.experimentoChave+'/participant/' + Questionario.idplayer + '/quiz/curso').once('value').then(function(snapshot) {
       var qualcurso = snapshot.val();;

       var qual_genero = firebase.database().ref("/experiment/" + Questionario.experimentoChave+'/participant/' + Questionario.idplayer + '/quiz/genero').once('value').then(function(snapshot) {
       var qualgenero = snapshot.val();;

       var qual_idade = firebase.database().ref("/experiment/" + Questionario.experimentoChave+'/participant/' + Questionario.idplayer + '/quiz/idade').once('value').then(function(snapshot) {
       var qualidade = snapshot.val();;

       var qual_inst = firebase.database().ref("/experiment/" + Questionario.experimentoChave+'/participant/' + Questionario.idplayer + '/quiz/instituicaoEnsino').once('value').then(function(snapshot) {
       var qualinst = snapshot.val();;

       var qual_nump = firebase.database().ref("/experiment/" + Questionario.experimentoChave+'/participant/' + Questionario.idplayer + '/quiz/numeroPessoas').once('value').then(function(snapshot) {
       var qualnump = snapshot.val();;

       var qual_renda = firebase.database().ref("/experiment/" + Questionario.experimentoChave+'/participant/' + Questionario.idplayer + '/quiz/renda').once('value').then(function(snapshot) {
